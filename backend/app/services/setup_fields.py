...
class SetupFieldService:
        ...
        for update in updates:
            ...
            if field.param_path is not None:
                if field.field_id == self._FIXED_IMPORT_PRICE_FIELD_ID:
                    ...
                    _set_payload_path(mutable_payload, field.param_path, normalized)
                event_id = create_setup_field_event(
                    db,
                    field_id=field.field_id,
                    source=update.source,
        ...
        if signal_values is not None:
            if not isinstance(signal_values, dict):
                ...
                for key, value in signal_values.items():
                    field_id = _signal_field_id_from_key(str(key))
                    if field_id is None:
                    ...
                    if error is not None:
                        ...
                        continue
                    self._ingest_signal(signal_key=signal_def.signal_key or key, value=normalized, source="import", ts=now)
    ...
    def _field_defs(self, payload: dict[str, Any]) -> list[SetupFieldDef]:
        ...
        return [
            ...
            SetupFieldDef("param.pvforecast.planes.0.inverter_paco", "mandatory", "PV Plane #1 Inverter PACO", True, "number", "W", None, 0, None, ("pvforecast", "planes", 0, "inverter_paco"), None, "/eos/set/param/pvforecast/planes/0/inverter_paco=<value>"),
            ...
            SetupFieldDef(self._FIXED_IMPORT_PRICE_FIELD_ID, "mandatory", "Bezugspreis fix (EOS Import-Serie)", True, "number", "EUR/kWh", None, 0, None, ("elecprice", "elecpriceimport", "import_json"), None, "/eos/set/param/elecprice/elecpriceimport/import_json/value=<value>"),
            ...
            SetupFieldDef("param.feedintariff.provider_settings.FeedInTariffFixed.feed_in_tariff_kwh", "mandatory", "Fester Einspeisetarif", True, "number", "EUR/kWh", None, 0, None, ("feedintariff", "provider_settings", "FeedInTariffFixed", "feed_in_tariff_kwh"), None, "/eos/set/param/feedintariff/provider_settings/FeedInTariffFixed/feed_in_tariff_kwh=<value>"),
            ...
            SetupFieldDef("param.devices.batteries.0.capacity_wh", "mandatory", "Batterie #1 Kapazität", True, "number", "Wh", None, 0, None, ("devices", "batteries", 0, "capacity_wh"), None, f"/eos/set/param/devices/batteries/{battery_selector}/capacity_wh=<value>"),
            SetupFieldDef("param.devices.batteries.0.min_charge_power_w", "mandatory", "Batterie #1 Min Charge", True, "number", "W", None, 0, None, ("devices", "batteries", 0, "min_charge_power_w"), None, f"/eos/set/param/devices/batteries/{battery_selector}/min_charge_power_w=<value>"),
            SetupFieldDef("param.devices.batteries.0.max_charge_power_w", "mandatory", "Batterie #1 Max Charge", True, "number", "W", None, 0, None, ("devices", "batteries", 0, "max_charge_power_w"), None, f"/eos/set/param/devices/batteries/{battery_selector}/max_charge_power_w=<value>"),
            ...
            SetupFieldDef("param.devices.inverters.0.max_power_w", "mandatory", "Inverter #1 Max Power", True, "number", "W", None, 0, None, ("devices", "inverters", 0, "max_power_w"), None, f"/eos/set/param/devices/inverters/{inverter_selector}/max_power_w=<value>"),
            ...
            SetupFieldDef("param.elecprice.charges_kwh", "optional", "Strompreis Zuschlag", False, "number", "EUR/kWh", None, 0, None, ("elecprice", "charges_kwh"), None, "/eos/set/param/elecprice/charges_kwh=<value>"),
            ...
            SetupFieldDef("signal.house_load_w", "live", "Hauslast", True, "number", "W", None, 0, 100000, None, "house_load_w", "/eos/set/signal/house_load_kw=<value>"),
            SetupFieldDef("signal.pv_power_w", "live", "PV Leistung", True, "number", "W", None, 0, 100000, None, "pv_power_w", "/eos/set/signal/pv_power_kw=<value>"),
            SetupFieldDef("signal.grid_import_w", "live", "Netzbezug", True, "number", "W", None, 0, 100000, None, "grid_import_w", "/eos/set/signal/grid_import_kw=<value>"),
            SetupFieldDef("signal.grid_export_w", "live", "Netzeinspeisung", True, "number", "W", None, 0, 100000, None, "grid_export_w", "/eos/set/signal/grid_export_kw=<value>"),
            SetupFieldDef("signal.battery_power_w", "live", "Batterieleistung", False, "number", "W", None, -100000, 100000, None, "battery_power_w", "/eos/set/signal/battery_power_kw=<value>"),
    ...
    def _load_signal_values_for_export(self, db: Session) -> dict[str, Any]:
        ...
        for row in rows:
            ...
            if not isinstance(key, str):
                continue
            signal_values[key] = _signal_row_value(row)
...
def _build_fixed_elecprice_import_json(*, payload: dict[str, Any], eur_per_kwh: float) -> str:
    ...
    import_payload = {
        ...
        "note": f"constant {eur_per_kwh} EUR/kWh expanded to {int(round(prediction_hours))}h x 15min",
    }
...
def _signal_field_id_from_key(signal_key: str) -> str | None:
    mapping = {
        ...
        "battery_power_w": "signal.battery_power_w",
        "battery_soc_pct": "signal.battery_soc_pct",
    }
    return mapping.get(signal_key)
...
def _param_path_to_field_id(*, param_path: str, payload: dict[str, Any]) -> str:
    ...
    static_paths = {
        "general/latitude": "param.general.latitude",
        "general/longitude": "param.general.longitude",
        "pvforecast/provider": "param.pvforecast.provider",
        "pvforecast/planes/0/peakpower": "param.pvforecast.planes.0.peakpower",
        "pvforecast/planes/0/surface_azimuth": "param.pvforecast.planes.0.surface_azimuth",
        "pvforecast/planes/0/surface_tilt": "param.pvforecast.planes.0.surface_tilt",
        "pvforecast/planes/0/inverter_paco": "param.pvforecast.planes.0.inverter_paco",
        "pvforecast/planes/0/loss": "param.pvforecast.planes.0.loss",
        "pvforecast/planes/0/trackingtype": "param.pvforecast.planes.0.trackingtype",
        "elecprice/provider": "param.elecprice.provider",
        "elecprice/elecpriceimport/import_json/value": "param.elecprice.elecpriceimport.import_json.value",
        "elecprice/charges_kwh": "param.elecprice.charges_kwh",
        "elecprice/vat_rate": "param.elecprice.vat_rate",
        "elecprice/energycharts/bidding_zone": "param.elecprice.energycharts.bidding_zone",
        "feedintariff/provider": "param.feedintariff.provider",
        "feedintariff/provider_settings/FeedInTariffFixed/feed_in_tariff_kwh": "param.feedintariff.provider_settings.FeedInTariffFixed.feed_in_tariff_kwh",
        "load/provider": "param.load.provider",
        "load/provider_settings/LoadAkkudoktor/loadakkudoktor_year_energy_kwh": "param.load.provider_settings.LoadAkkudoktor.loadakkudoktor_year_energy_kwh",
        "measurement/keys": "param.measurement.keys",
        "measurement/load_emr_keys": "param.measurement.load_emr_keys",
        "measurement/grid_import_emr_keys": "param.measurement.grid_import_emr_keys",
        "measurement/grid_export_emr_keys": "param.measurement.grid_export_emr_keys",
        "measurement/pv_production_emr_keys": "param.measurement.pv_production_emr_keys",
    }
    ...
    if len(parts) == 4 and parts[:2] == ["devices", "batteries"]:
        ...
        return f"param.devices.batteries.{index}.{field_name}"
    ...
    if len(parts) == 4 and parts[:2] == ["devices", "inverters"]:
        ...
        if field_name not in {"device_id", "max_power_w", "battery_id"}:
            raise ValueError("Unsupported inverter field")
        return f"param.devices.inverters.{index}.{field_name}"
