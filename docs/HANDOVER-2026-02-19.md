# EOS-Webapp Handover (updated 2026-02-20)
...
### Active app mode
- Current functional state is **Slice 9**:
...
- UI unit policy is fixed for operators:
  ...
  - capacity in `kWh`
  - power in `kW`
- Legacy channel/mapping/binding endpoints are intentionally deprecated and return `410 Gone`.
...
- Slice docs are chronological; older slices describe historical stages and can conflict with the current HTTP-only mode.
